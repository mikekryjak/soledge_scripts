...
def assign_triangles_to_trans_mesh(Root, WallTriangles, RKnots, ZKnots):

	if(DEBUG > 0): print("assign_triangles_to_trans_mesh")
	...
	for k in range(nZones):
		...
		Zones[k].list_tri_nnums = np.zeros((Nx, Nz), dtype='i4')
		Zones[k].list_tri_nums	= np.zeros((Nx, Nz, 16), dtype='i4')
		...
		if len(ii) > 0:
			for l in range(len(ii)):
				...
				while(not PointOK):
					...
					ListTri = np.empty(2*(2*ncase+1), dtype='i4')
					...
					for n in range(ncase):
						...
						i_2 = i_
						j_2 = j_
						k_  = Zones[k_2].point_west_k[i_2,j_2]
						i_  = Zones[k_2].point_west_i[i_2,j_2]
						j_  = Zones[k_2].point_west_j[i_2,j_2]
					...
					for n in range(ncase):
						...
						i_2 = i_
						j_2 = j_
						k_  = Zones[k_2].point_east_k[i_2,j_2]
						i_  = Zones[k_2].point_east_i[i_2,j_2]
						j_  = Zones[k_2].point_east_j[i_2,j_2]
						if(k_ == -1): break									#test proximity
					...
					for n in range(List.shape[0]):
						iTri = np.where((WallTriangles.k == List[n,0]) & (WallTriangles.i == List[n,1]) & (WallTriangles.j == List[n,2]))[0]
						if(len(iTri) > 0):
							nTri = len(iTri)
...
#						look at 2D vicinity last chance
					...
					if(nListTri > 0):
						...
						Zones[k].list_tri_nnums[ii[l],jj[l]] 			= nListTri
						Zones[k].list_tri_nums[ii[l],jj[l],:] 			= -1
						Zones[k].list_tri_nums[ii[l],jj[l],0:nListTri]  = ListTri

					elif(ncase > 50):
						print("\tError no triangle found on west-east direction!")
	...
	if(DEBUG > 2):
		print("\tplot nodes & first found triangle in quadrangles crossing wall poloidal directon")
