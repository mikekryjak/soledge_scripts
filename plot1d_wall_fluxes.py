...
def plot1d_wall_fluxes(path="./", rz0_line = [0.,0.], theta_line=0., log_scale=0, one_plot=0, no_samex=0, save="none"):
...
#	Read wall
	...
	if_mesh = h5py.File(str(path)+"/mesh.h5", "r")
...
#	Read Eirene mesh
	...
	if_tri	 = h5py.File(str(path)+"/triangles.h5", "r")
	...
	try:
		AFluxF	 = np.loadtxt(path+"soledge2D.atoms_fluxes_wall_1", dtype='f8')
...
#					 8:Flux_Atom_Emitted_from_Atom, 9:Flux_Mol_Emitted_from_Atom]
	...
	iTri	= iTri.astype(np.int)
	iSide	= iSide.astype(np.int)
	...
	try:
		FluxiF	 = np.loadtxt(path+"soledge2D.ion_fluxes_wall_1", dtype='f8')
...
#					 4:Flux_Ion_Incident, 			5:Flux_Atom_Emitted_from_Ion]
	...
	FluxiN=[]
	try:
		FluxiN.append(np.loadtxt(path+"soledge2D.atoms_fluxes_wall_1", dtype='f8'))
#            		[0:iTri, 1:iSide, 2:iProp, 3:dlWall, 
#					 4:Flux_Atom_Incident, 			5:Flux_Mol_Incident,
#					 6:Flux_Atom_Emitted_from_Ion, 	7:Flux_Mol_Emitted_from_Ion,
#					 8:Flux_Atom_Emitted_from_Atom, 9:Flux_Mol_Emitted_from_Atom]
	...
	try:
		details = False
		if(details):
			EFluxF	 = np.loadtxt(path+"soledge2D.energy_fluxes_details_1", dtype='f8', comments="%")
#			[0:dlWall, 
...
#			7:Flux_Energy_Recombination_in_Wall,8:Flux_Energy_incident_Molecules]
			...
			EFluxF	 = np.loadtxt(path+"soledge2D.energy_fluxes_1", dtype='f8', comments="%")
#           [0:iTri, 1:iSide, 2:iProp, 3:dlWall, 
